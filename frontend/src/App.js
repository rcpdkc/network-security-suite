...
const SettingsView = ({ API_URL }) => {
  const [activeTab, setActiveTab] = useState('sshTemplates'); // 'apiTemplates' | 'sshTemplates' | 'snmpTemplates' | 'discoveryTemplates' | 'cveDb' | 'kb' | 'switchKb' | 'ldap' | 'certs'
  ...
  const [loadedTabs, setLoadedTabs] = useState({ customIcons: false, apiTemplates: false, sshTemplates: false, snmpTemplates: false, discoveryTemplates: false, cveDb: false, kb: false, switchKb: false, ldap: false, certs: false });
  ...
  return (
    <div className="settings-view fade-in">
      <div style={{display:'flex', gap:'20px', marginBottom:'30px', borderBottom:'1px solid #e2e8f0', paddingBottom:'15px', flexWrap:'wrap'}}>
        ...
        <button onClick={() => setActiveTab('discoveryTemplates')} style={{padding:'10px 20px', borderRadius:'10px', border:'none', background: activeTab==='discoveryTemplates'?'var(--primary)':'transparent', color: activeTab==='discoveryTemplates'?'white':'#64748b', fontWeight:'700', cursor:'pointer', display:'flex', alignItems:'center', gap:'8px'}}><Search size={18}/> Keşif Şablonları</button>
      ...
      {activeTab === 'discoveryTemplates' && (
        <div className="discovery-templates-manager fade-in">
          <DiscoveryTemplatesManager API_URL={API_URL} />
        </div>
      )}
...
const HowToView = ({ API_URL }) => {
  ...
  const [viewingHowTo, setViewingHowTo] = useState(null);
  const [editingHowTo, setEditingHowTo] = useState(null);
  ...
  const fetchHowTos = async () => {
    try {
      const res = await axios.get(`${API_URL}/howtos`);
      setHowtos(res.data);
    } catch (e) { console.error(e); }
  };

  useEffect(() => { fetchHowTos(); }, []);

  const handleAddStep = () => {
    setForm({
      ...form,
      steps: [...form.steps, { id: uuidv4(), title: '', content: '', image: '', isBold: false, isItalic: false }]
    });
  };
  ...
  const handleImageUpload = (id, e) => {
    ...
    if (!file) return;
    const reader = new FileReader();
    reader.onloadend = () => { handleStepChange(id, 'image', reader.result); };
    reader.readAsDataURL(file);
  };
  ...
  const handleSave = async (e) => {
    e.preventDefault();
    try {
      if (editingHowTo) { await axios.put(`${API_URL}/howtos/${editingHowTo.id}`, form); }
      else { await axios.post(`${API_URL}/howtos`, form); }
  ...
  const handleDelete = async (e, id) => {
    ...
    try {
      await axios.delete(`${API_URL}/howtos/${id}`);
      fetchHowTos();
      if (viewingHowTo?.id === id) setViewingHowTo(null);
    } catch (e) { alert('Silme hatası!'); }
  };

  const toggleStepImage = (stepId) => {
    setActiveImageSteps(prev => 
  ...
  if (viewingHowTo) {
    return (
      <div className="howto-detail-view fade-in" style={{ maxWidth: '1000px', margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '20px', marginBottom: '30px' }}>
          <button onClick={() => { setViewingHowTo(null); setActiveImageSteps([]); }} style={{ background: 'white', border: '1px solid #e2e8f0', borderRadius: '12px', padding: '10px 15px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', color: '#64748b', fontWeight: '700' }}><ArrowLeft size={18} /> Geri Dön</button>
          <div>
            <h2 style={{ margin: 0, fontSize: '1.75rem', fontWeight: '900', color: '#0f172a' }}>{viewingHowTo.title}</h2>
            <p style={{ margin: '5px 0 0', color: '#64748b', fontSize: '15px' }}>{viewingHowTo.description}</p>
          </div>
        </div>
        <div style={{ display: 'grid', gap: '20px' }}>
          {viewingHowTo.steps.map((step, idx) => {
            const stepId = step.id || idx;
            const isImageOpen = activeImageSteps.includes(stepId);
            const hasImage = !!step.image;
            return (
              <div key={stepId} style={{ background: 'white', borderRadius: '24px', border: '1px solid #e2e8f0', overflow: 'hidden', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)' }}>
                <div style={{ display: 'flex', gap: '30px', padding: '30px' }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: '12px', fontWeight: '900', color: 'var(--primary)', marginBottom: '10px', textTransform: 'uppercase', letterSpacing: '1px' }}>ADIM {idx + 1}</div>
                    <h3 style={{ fontWeight: '800', fontSize: '1.25rem', color: '#1e293b', marginBottom: '15px' }}>{step.title}</h3>
                    <div style={{ fontSize: '16px', color: '#475569', fontWeight: step.isBold ? 'bold' : 'normal', fontStyle: step.isItalic ? 'italic' : 'normal', lineHeight: '1.7' }}>{step.content}</div>
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', borderLeft: '1px solid #f1f5f9', paddingLeft: '30px', minWidth: '100px' }}>
                    <button onClick={() => hasImage && toggleStepImage(stepId)} style={{ background: isImageOpen ? '#eff6ff' : 'none', border: 'none', cursor: hasImage ? 'pointer' : 'default', color: hasImage ? (isImageOpen ? 'var(--primary)' : '#0369a1') : '#ef4444', padding: '15px', borderRadius: '20px', transition: 'all 0.2s', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px' }}>
                      <Image size={32} strokeWidth={hasImage ? 2.5 : 1.5} />
                      <span style={{ fontSize: '11px', fontWeight: '900' }}>{hasImage ? (isImageOpen ? 'GİZLE' : 'GÖRSEL') : 'YOK'}</span>
                    </button>
                  </div>
                </div>
                {isImageOpen && hasImage && (
                  <div style={{ padding: '0 30px 30px', borderTop: '1px dashed #e2e8f0', background: '#f8fafc' }}>
                    <div style={{ marginTop: '20px', borderRadius: '16px', overflow: 'hidden', border: '1px solid #e2e8f0', background: 'white' }}>
                      <img src={step.image} alt={step.title} style={{ width: '100%', maxHeight: '600px', objectFit: 'contain', display: 'block' }} />
                    </div>
                  </div>
                )}
              </div>
  ...
  return (
    <div className="howto-view fade-in">
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '20px' }}>
        <button onClick={() => { setEditingHowTo(null); setForm({ title: '', description: '', steps: [] }); setIsModalOpen(true); }} style={{ background: 'var(--primary)', color: 'white', border: 'none', padding: '12px 25px', borderRadius: '12px', fontWeight: '700', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '10px' }}><Plus size={18} /> Yeni Rehber Ekle</button>
      </div>
      <div style={{ display: 'grid', gap: '20px', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))' }}>
        {howtos.map((h) => (
          <div key={h.id} onClick={() => { setViewingHowTo(h); setActiveImageSteps([]); }} style={{ background: 'white', padding: '25px', borderRadius: '24px', border: '1px solid #e2e8f0', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)', display: 'flex', flexDirection: 'column', cursor: 'pointer', transition: 'all 0.3s ease', position: 'relative', minHeight: '140px', justifyContent: 'center', alignItems: 'center', textAlign: 'center' }}>
            <div style={{ background: '#f0f9ff', color: '#0369a1', padding: '12px', borderRadius: '16px', marginBottom: '15px' }}><FileText size={24} /></div>
            <h4 style={{ fontSize: '16px', fontWeight: '800', color: '#1e293b', margin: 0 }}>{h.title}</h4>
            <div style={{ position: 'absolute', top: '15px', right: '15px', display: 'flex', gap: '5px' }}>
              <button onClick={(e) => { e.stopPropagation(); setEditingHowTo(h); setForm(h); setIsModalOpen(true); }} style={{ padding: '6px', borderRadius: '8px', border: 'none', background: '#f1f5f9', cursor: 'pointer', color: '#64748b' }}><Edit size={14} /></button>
              <button onClick={(e) => handleDelete(e, h.id)} style={{ padding: '6px', borderRadius: '8px', border: 'none', background: '#fee2e2', color: '#ef4444', cursor: 'pointer' }}><Trash2 size={14} /></button>
            </div>
          </div>
        ))}
        {howtos.length === 0 && <div style={{ gridColumn: '1/-1', textAlign: 'center', padding: '60px', color: '#94a3b8', background: 'white', borderRadius: '24px' }}>Henüz bir rehber eklenmedi.</div>}
      </div>
      {isModalOpen && (
        <div className="modal-overlay" style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(15,23,42,0.6)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1200 }}>
          <div style={{ background: 'white', width: '800px', maxWidth: '96vw', maxHeight: '90vh', borderRadius: '24px', padding: '30px', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.25)', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              <h3 style={{ margin: 0, fontSize: '1.25rem', fontWeight: '800' }}>{editingHowTo ? 'Rehberi Düzenle' : 'Yeni Rehber Oluştur'}</h3>
            ...
            <form onSubmit={handleSave} style={{ display: 'grid', gap: '20px' }}>
              <div><label style={{ fontSize: '12px', fontWeight: '700', color: '#64748b', display: 'block', marginBottom: '5px' }}>REHBER BAŞLIĞI</label><input type="text" value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} style={{ width: '100%', padding: '12px', borderRadius: '10px', border: '1px solid #e2e8f0' }} required placeholder="Örn: FortiGate VLAN Oluşturma" /></div>
              <div><label style={{ fontSize: '12px', fontWeight: '700', color: '#64748b', display: 'block', marginBottom: '5px' }}>GENEL AÇIKLAMA</label><textarea value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} style={{ width: '100%', padding: '12px', borderRadius: '10px', border: '1px solid #e2e8f0', minHeight: '80px' }} placeholder="Bu rehber ne hakkında?" /></div>
              <div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}><label style={{ fontSize: '12px', fontWeight: '700', color: '#64748b' }}>ADIMLAR (MADDELER)</label><button type="button" onClick={handleAddStep} style={{ background: '#f1f5f9', color: 'var(--primary)', border: 'none', padding: '6px 12px', borderRadius: '8px', fontSize: '12px', fontWeight: '700', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '5px' }}><Plus size={14} /> Adım Ekle</button></div>
                <div style={{ display: 'grid', gap: '15px' }}>
                  {form.steps.map((step, index) => (
                    <div key={step.id} style={{ padding: '20px', background: '#f8fafc', borderRadius: '16px', border: '1px solid #e2e8f0', position: 'relative' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '15px' }}>
                        <span style={{ fontSize: '12px', fontWeight: '800', color: 'var(--primary)' }}>ADIM {index + 1}</span>
                        <div style={{ display: 'flex', gap: '5px' }}>
                          <button type="button" onClick={() => moveStep(index, -1)} disabled={index === 0} style={{ padding: '4px', borderRadius: '6px', border: '1px solid #e2e8f0', background: 'white', cursor: index === 0 ? 'default' : 'pointer', opacity: index === 0 ? 0.5 : 1 }}><ChevronUp size={16} /></button>
                          <button type="button" onClick={() => moveStep(index, 1)} disabled={index === form.steps.length - 1} style={{ padding: '4px', borderRadius: '6px', border: '1px solid #e2e8f0', background: 'white', cursor: index === form.steps.length - 1 ? 'default' : 'pointer', opacity: index === form.steps.length - 1 ? 0.5 : 1 }}><ChevronDown size={16} /></button>
                          <button type="button" onClick={() => handleRemoveStep(step.id)} style={{ padding: '4px', borderRadius: '6px', border: '1px solid #fee2e2', background: 'white', color: '#ef4444', cursor: 'pointer' }}><Trash2 size={16} /></button>
                        </div>
                      </div>
                      <div style={{ display: 'grid', gap: '10px' }}>
                        <input type="text" placeholder="Adım Başlığı" value={step.title} onChange={e => handleStepChange(step.id, 'title', e.target.value)} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid #e2e8f0', fontSize: '14px', fontWeight: '700' }} />
                        <div style={{ display: 'flex', gap: '5px', marginBottom: '5px' }}>
                          <button type="button" onClick={() => handleStepChange(step.id, 'isBold', !step.isBold)} style={{ padding: '5px 10px', borderRadius: '4px', border: '1px solid #e2e8f0', background: step.isBold ? '#e2e8f0' : 'white', fontWeight: 'bold' }}>B</button>
                          <button type="button" onClick={() => handleStepChange(step.id, 'isItalic', !step.isItalic)} style={{ padding: '5px 10px', borderRadius: '4px', border: '1px solid #e2e8f0', background: step.isItalic ? '#e2e8f0' : 'white', fontStyle: 'italic' }}>I</button>
                        </div>
                        <textarea placeholder="Adım açıklaması..." value={step.content} onChange={e => handleStepChange(step.id, 'content', e.target.value)} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid #e2e8f0', minHeight: '60px', fontSize: '13px', fontWeight: step.isBold ? 'bold' : 'normal', fontStyle: step.isItalic ? 'italic' : 'normal' }} />
                        <div style={{ display: 'grid', gridTemplateColumns: step.image ? '100px 1fr' : '1fr', gap: '15px', alignItems: 'center' }}>
                          {step.image && <img src={step.image} alt="Preview" style={{ width: '100px', height: '60px', objectFit: 'cover', borderRadius: '8px' }} />}
                          <div style={{ position: 'relative' }}>
                            <input type="file" accept="image/*" onChange={(e) => handleImageUpload(step.id, e)} style={{ display: 'none' }} id={`img-${step.id}`} />
                            <label htmlFor={`img-${step.id}`} style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', background: 'white', border: '1px solid #e2e8f0', padding: '8px 15px', borderRadius: '8px', fontSize: '12px', fontWeight: '700', cursor: 'pointer' }}><Upload size={14} /> {step.image ? 'Resmi Değiştir' : 'Resim Ekle'}</label>
                            {step.image && <button type="button" onClick={() => handleStepChange(step.id, 'image', '')} style={{ marginLeft: '10px', color: '#ef4444', background: 'none', border: 'none', fontSize: '12px', cursor: 'pointer' }}>Kaldır</button>}
                          </div>
                        </div>
                      </div>
                    </div>
                  ))}
                  {form.steps.length === 0 && <div style={{ textAlign: 'center', padding: '20px', border: '2px dashed #e2e8f0', borderRadius: '16px', color: '#94a3b8', fontSize: '13px' }}>Henüz bir adım eklenmedi.</div>}
                </div>
              </div>
              <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end', marginTop: '10px', padding: '20px 0', borderTop: '1px solid #f1f5f9' }}>
                <button type="button" onClick={() => setIsModalOpen(false)} style={{ padding: '12px 25px', borderRadius: '12px', border: '1px solid #e2e8f0', background: 'white', fontWeight: '700', cursor: 'pointer' }}>İptal</button>
                <button type="submit" style={{ padding: '12px 35px', borderRadius: '12px', border: 'none', background: 'var(--primary)', color: 'white', fontWeight: '700', cursor: 'pointer' }}>{editingHowTo ? 'Değişiklikleri Kaydet' : 'Rehberi Oluştur'}</button>
              </div>
            </form>
          </div>
        </div>
...
const DeviceManagementView = ({ API_URL }) => {
  ...
  const getDeviceMethodLabel = (device) => {
    const hasApi = hasApiConfig(device);
    const hasSnmpSsh = hasSnmpSshConfig(device);
    if (hasApi && hasSnmpSsh) return 'API + SNMP + SSH';
  ...
  const handleTest = async () => {
    ...
    try {
      const payload = {
        ...form,
        snmp_template_id: form.snmp_template_id === '' ? null : Number(form.snmp_template_id),
        ssh_template_id: form.ssh_template_id === '' ? null : Number(form.ssh_template_id),
        api_template_id: form.api_template_id === '' ? null : Number(form.api_template_id)
      };
      const res = await axios.post(`${API_URL}/devices/test-connection`, payload);
      setTestResult({ success: true, message: res.data.message, details: res.data.details || null });
      ...
      setTestResult({
        success: false,
        message: responseData?.message || responseData?.error || e.message,
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        await axios.put(`${API_URL}/devices/${editingId}`, payload);
      ...
      <div style={{background:'white', borderRadius:'24px', border:'1px solid #e2e8f0', overflow:'hidden', boxShadow:'0 4px 6px -1px rgba(0,0,0,0.05)'}}>
        <table style={{width:'100%', borderCollapse:'collapse'}}>
          ...
          <tbody>
              ...
              <tr key={d.id} style={{borderBottom:'1px solid #f1f5f9'}}>
                ...
                <td style={{padding:'15px 25px'}}>
                  <div style={{display:'flex', flexDirection:'column'}}>
                    ...
                    <span style={{fontSize:'11px', color:'#64748b'}}>{d.vendor || 'N/A'}</span>
                  </div>
                </td>
                <td style={{padding:'15px 25px'}}>
                ...
                <td style={{padding:'15px 25px', textAlign:'right'}}>
                  <button onClick={() => {
                    setForm({
                      ...
                      device_type: d.device_type || 'Firewall',
                      vendor: d.vendor || 'Fortinet',
                      connection_method: hasApiConfig(d) ? 'api' : 'snmp_ssh',
            ...
            {devices.length === 0 && <tr><td colSpan="6" style={{padding:'40px', textAlign:'center', color:'#94a3b8'}}>Henüz bir cihaz eklenmedi.</td></tr>}
          </tbody>
        </table>
      </div>
        ...
        <div className="modal-overlay" style={{position:'fixed', top:0, left:0, right:0, bottom:0, background:'rgba(15,23,42,0.6)', backdropFilter:'blur(4px)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1000}}>
          <div style={{background:'white', width:'650px', maxWidth:'96vw', borderRadius:'24px', padding:'35px', boxShadow:'0 25px 50px -12px rgba(0,0,0,0.25)'}}>
            ...
            <form onSubmit={handleSave} style={{display:'grid', gap:'20px'}}>
              ...
              <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'20px'}}>
                <div>
                  <label style={{fontSize:'12px', fontWeight:'700', color:'#64748b'}}>CİHAZ TÜRÜ</label>
                  <select value={form.device_type} onChange={e => setForm({...form, device_type: e.target.value})} style={{width:'100%', padding:'12px', borderRadius:'12px', border:'1px solid #e2e8f0', background:'white'}}>
                ...
                <div>
                  <label style={{fontSize:'12px', fontWeight:'700', color:'#64748b'}}>MARKA (VENDOR)</label>
                  <select value={form.vendor} onChange={e => setForm({...form, vendor: e.target.value})} style={{width:'100%', padding:'12px', borderRadius:'12px', border:'1px solid #e2e8f0', background:'white'}}>
              ...
              <div style={{padding:'20px', borderRadius:'15px', background: testResult ? (testResult.success ? '#f0fdf4' : '#fef2f2') : '#f8fafc', border: testResult ? (testResult.success ? '1px solid #bbf7d0' : '1px solid #fecaca') : '1px solid #e2e8f0'}}>
                ...
                {testResult?.details && form.connection_method === 'snmp_ssh' && (
                  <div style={{marginTop:'10px', display:'grid', gap:'6px'}}>
                    <div style={{fontSize:'12px', color: testResult.details.snmp?.success ? '#166534' : '#991b1b'}}>
                      SNMP: {testResult.details.snmp?.success ? 'Başarılı' : 'Başarısız'}{testResult.details.snmp?.message ? ` - ${testResult.details.snmp.message}` : ''}
                    </div>
                    <div style={{fontSize:'12px', color: testResult.details.ssh?.success ? '#166534' : '#991b1b'}}>
                      SSH: {testResult.details.ssh?.success ? 'Başarılı' : 'Başarısız'}{testResult.details.ssh?.message ? ` - ${testResult.details.ssh.message}` : ''}
                    </div>
                  </div>
                )}
              </div>
...
const DeepDiscoveryView = ({ API_URL }) => {
  const [devices, setDevices] = useState([]);
  ...
  const fetchData = async () => {
    try {
      const [devRes, resultRes, tplRes] = await Promise.all([
        axios.get(`${API_URL}/deep-discovery/devices`),
        axios.get(`${API_URL}/deep-discovery/results?limit=300`),
        axios.get(`${API_URL}/discovery-templates`)
      ]);
  ...
  // Polling for logs while discovery is running
  useEffect(() => {
    let interval;
    if (activeRunId) {
      interval = setInterval(async () => {
        try {
          const res = await axios.get(`${API_URL}/deep-discovery/runs/${activeRunId}`);
          ...
          if (res.data.status !== 'running') {
            ...
            if (res.data.status === 'completed') {
              alert(`Keşif tamamlandı. ${res.data.discovered_count} kayıt bulundu.`);
            } else if (res.data.status === 'cancelled') {
              alert('Keşif iptal edildi.');
            } else if (res.data.status === 'error') {
              alert('Keşif sırasında hata oluştu.');
            }
          }
  ...
  const handleStartDiscovery = async () => {
    ...
    const missingBrand = selectedDevices.filter(d => !d.vendor);
    
    if (missingBrand.length > 0) {
      alert(`⚠️ Hata: Seçilen ${missingBrand.length} cihazın marka (Vendor) bilgisi eksik. Lütfen Cihaz Yönetimi'nden marka bilgisini güncelleyin.\n\nEksik cihazlar: ${missingBrand.map(d => d.ip_address).join(', ')}`);
      return;
    }
    ...
    try {
      // Backend handles matching by vendor if template_id is null or we can send per-device templates
      // For now, let's trigger discovery for each device with its matching template or let backend do it
      // I'll update the backend start endpoint to support 'auto' template matching
      const res = await axios.post(`${API_URL}/deep-discovery/start`, {
        device_ids: selectedDeviceIds,
        template_id: null // Tell backend to use Smart Matching
      });
      setActiveRunId(res.data.run_id);
    } catch (e) {
      alert('Keşif başlatılamadı: ' + (e.response?.data?.error || e.message));
      setLoading(false);
    }
  };
  ...
  const handleCancelDiscovery = async () => {
    ...
    if (!window.confirm('Derin keşif işlemini iptal etmek istediğinize emin misiniz?')) return;
    
    try {
      await axios.post(`${API_URL}/deep-discovery/cancel/${activeRunId}`);
      alert('İptal isteği gönderildi.');
    } catch (e) {
      alert('İptal hatası: ' + (e.response?.data?.error || e.message));
    }
  };
    ...
    <div className="fade-in" style={{display:'grid', gap:'20px'}}>
      <div style={{background:'white', borderRadius:'20px', border:'1px solid #e2e8f0', padding:'20px'}}>
        <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', gap:'12px', marginBottom:'12px', flexWrap:'wrap'}}>
          <div>
            <h3 style={{margin:'0 0 5px', fontSize:'1.1rem', fontWeight:'800', color:'#0f172a'}}>Derin Ağ Keşfi (Smart Discovery)</h3>
            <p style={{margin:0, color:'#64748b', fontSize:'13px'}}>Cihazların türü ve markasına göre otomatik şablon eşleştirilir.</p>
          </div>
          <div style={{display:'flex', gap:'12px', alignItems:'center', flexWrap:'wrap'}}>
            {loading && activeRunId && (
              <button onClick={handleCancelDiscovery} style={{background:'#ef4444', color:'white', border:'none', padding:'11px 18px', borderRadius:'10px', fontWeight:'800', cursor: 'pointer', whiteSpace:'nowrap', display:'flex', alignItems:'center', gap:'8px'}}>
                <XCircle size={16} />
                İptal Et
              </button>
            )}
            <button onClick={handleStartDiscovery} disabled={loading} style={{background:'var(--primary)', color:'white', border:'none', padding:'11px 18px', borderRadius:'10px', fontWeight:'800', cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? 0.7 : 1, whiteSpace:'nowrap', display:'flex', alignItems:'center', gap:'8px'}}>
              {loading ? <RefreshCw size={16} className="spin" /> : <Zap size={16} />}
              {loading ? 'Keşif Çalışıyor...' : 'Akıllı Keşif Başlat'}
            </button>
          </div>
        </div>

        <div style={{maxHeight:'260px', overflowY:'auto', border:'1px solid #e2e8f0', borderRadius:'14px'}}>
          <table style={{width:'100%', borderCollapse:'collapse'}}>
            <thead style={{background:'#f8fafc'}}>
              <tr>
                <th style={{padding:'10px 12px', textAlign:'left', fontSize:'12px', color:'#64748b'}}>SEÇ</th>
                <th style={{padding:'10px 12px', textAlign:'left', fontSize:'12px', color:'#64748b'}}>CİHAZ / IP</th>
                <th style={{padding:'10px 12px', textAlign:'left', fontSize:'12px', color:'#64748b'}}>ÜRÜN / MARKA</th>
                <th style={{padding:'10px 12px', textAlign:'left', fontSize:'12px', color:'#64748b'}}>SNMP</th>
                <th style={{padding:'10px 12px', textAlign:'left', fontSize:'12px', color:'#64748b'}}>SSH</th>
              </tr>
            </thead>
            <tbody>
              {devices.map((d) => (
                <tr key={d.id} style={{borderTop:'1px solid #f1f5f9'}}>
                  <td style={{padding:'10px 12px'}}>
                    <input type="checkbox" checked={selectedDeviceIds.includes(d.id)} onChange={() => toggleDeviceSelection(d.id)} />
                  </td>
                  <td style={{padding:'10px 12px'}}>
                    <div style={{fontWeight:'700', color:'#1e293b'}}>{d.name || '-'}</div>
                    <div style={{fontSize:'11px', color:'#64748b'}}>{d.ip_address}</div>
                  </td>
                  <td style={{padding:'10px 12px'}}>
                    {d.vendor ? (
                      <div>
                        <div style={{fontSize:'12px', fontWeight:'600', color:'#1e293b'}}>{d.device_type}</div>
                        <div style={{fontSize:'11px', color:'#64748b'}}>{d.vendor}</div>
                      </div>
                    ) : (
                      <div style={{fontSize:'11px', color:'#ef4444', fontWeight:'700'}}>⚠️ Marka Belirtilmemiş</div>
                    )}
                  </td>
                  <td style={{padding:'10px 12px', color:d.snmp_template_id ? '#166534' : '#991b1b', fontWeight:'700'}}>{d.snmp_template_id ? (d.snmp_template_name || 'Tanımlı') : 'Yok'}</td>
                  <td style={{padding:'10px 12px', color:d.ssh_template_id ? '#166534' : '#991b1b', fontWeight:'700'}}>{d.ssh_template_id ? (d.ssh_template_name || 'Tanımlı') : 'Yok'}</td>
                </tr>
              ))}
              {devices.length === 0 && <tr><td colSpan="5" style={{padding:'20px', textAlign:'center', color:'#94a3b8'}}>Cihaz bulunamadı.</td></tr>}
            </tbody>
          </table>
        </div>

        {summary.length > 0 && (
          <div style={{marginTop:'12px', padding:'12px', border:'1px solid #e2e8f0', borderRadius:'12px', background:'#f8fafc', display:'grid', gap:'8px'}}>
            <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'8px', paddingBottom:'8px', borderBottom:'1px solid #cbd5e1'}}>
              <span style={{fontWeight:'700', color:'#0f172a'}}>📊 Keşif Özeti</span>
              <button onClick={() => setShowLogs(true)} style={{padding:'6px 12px', background:'#f0f9ff', color:'#0369a1', border:'1px solid #bae6fd', borderRadius:'6px', fontWeight:'700', cursor:'pointer', fontSize:'12px'}}>
                📋 Günlük ({detailedLogs.length})
              </button>
            </div>
            {summary.map((s) => (
              <div key={s.device_id} style={{fontSize:'12px', color:'#334155'}}>
                <strong>{s.device_name || s.ip_address}</strong> - SNMP: {s.snmp?.message || '-'} | SSH: {s.ssh?.message || '-'}
              </div>
            ))}
          </div>
        )}
      ...
      <div style={{background:'white', borderRadius:'20px', border:'1px solid #e2e8f0', overflow:'hidden'}}>
        <div style={{padding:'16px 18px', borderBottom:'1px solid #e2e8f0', background:'#f8fafc'}}>
          <h3 style={{margin:0, fontSize:'1rem', fontWeight:'800', color:'#0f172a'}}>Keşfedilen Cihaz Kayıtları</h3>
        </div>
        <div style={{overflowX:'auto'}}>
          <table style={{width:'100%', borderCollapse:'collapse'}}>
            <thead style={{background:'#f8fafc'}}>
              <tr>
                <th style={{padding:'10px 12px', textAlign:'left', fontSize:'12px', color:'#64748b'}}>KAYNAK CİHAZ</th>
                <th style={{padding:'10px 12px', textAlign:'left', fontSize:'12px', color:'#64748b'}}>PROTOKOL</th>
                <th style={{padding:'10px 12px', textAlign:'left', fontSize:'12px', color:'#64748b'}}>HOSTNAME</th>
                <th style={{padding:'10px 12px', textAlign:'left', fontSize:'12px', color:'#64748b'}}>IP</th>
                <th style={{padding:'10px 12px', textAlign:'left', fontSize:'12px', color:'#64748b'}}>MAC</th>
                <th style={{padding:'10px 12px', textAlign:'left', fontSize:'12px', color:'#64748b'}}>PORT</th>
                <th style={{padding:'10px 12px', textAlign:'left', fontSize:'12px', color:'#64748b'}}>KEŞİF ZAMANI</th>
              </tr>
            </thead>
            <tbody>
              {results.map((r) => (
                <tr key={r.id} style={{borderTop:'1px solid #f1f5f9'}}>
                  <td style={{padding:'10px 12px', fontWeight:'700', color:'#1e293b'}}>{r.source_device_name || r.source_device_ip || '-'}</td>
                  <td style={{padding:'10px 12px', color:'#475569'}}>{r.protocol || '-'}</td>
                  <td style={{padding:'10px 12px', color:'#475569'}}>{r.discovered_hostname || '-'}</td>
                  <td style={{padding:'10px 12px', color:'#475569'}}>{r.discovered_ip || '-'}</td>
                  <td style={{padding:'10px 12px', color:'#475569'}}>{r.discovered_mac || '-'}</td>
                  <td style={{padding:'10px 12px', color:'#475569'}}>{r.discovered_on_port || '-'}</td>
                  <td style={{padding:'10px 12px', color:'#475569'}}>{r.created_at ? new Date(r.created_at).toLocaleString('tr-TR') : '-'}</td>
                </tr>
              ))}
              {results.length === 0 && <tr><td colSpan="7" style={{padding:'24px', textAlign:'center', color:'#94a3b8'}}>Henüz keşif sonucu yok.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
      ...
      {showLogs && (
        <div style={{position:'fixed', top:0, left:0, right:0, bottom:0, background:'rgba(15,23,42,0.7)', backdropFilter:'blur(4px)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:10000}}>
          <div style={{background:'white', width:'95vw', maxWidth:'1200px', maxHeight:'90vh', borderRadius:'24px', padding:'24px', boxShadow:'0 25px 50px -12px rgba(0,0,0,0.25)', display:'flex', flexDirection:'column'}}>
            <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'16px'}}>
              <h3 style={{margin:0, fontSize:'1.2rem', fontWeight:'800', color:'#0f172a'}}>Keşif İşlem Günlüğü</h3>
              <div style={{display:'flex', gap:'12px', alignItems:'center'}}>
                {loading && (
                  <button onClick={handleCancelDiscovery} style={{background:'#fee2e2', color:'#991b1b', border:'1px solid #fecaca', padding:'8px 16px', borderRadius:'10px', fontWeight:'800', cursor:'pointer', display:'flex', alignItems:'center', gap:'8px', fontSize:'13px'}}>
                    <XCircle size={16} />
                    İptal Et
                  </button>
                )}
                <button onClick={() => !loading && setShowLogs(false)} style={{background:'none', border:'none', cursor: loading ? 'wait' : 'pointer', padding:'8px', fontSize:'20px', color:'#64748b'}}>✕</button>
              </div>
            </div>
            {loading && (
              <div style={{marginBottom:'10px', padding:'10px 12px', borderRadius:'8px', background:'#eff6ff', color:'#1d4ed8', fontSize:'12px', fontWeight:'700', display:'flex', alignItems:'center', gap:'10px'}}>
                <RefreshCw size={14} className="spin" /> Keşif çalışıyor... Cihazlara bağlanılıyor ve veriler çekiliyor.
              </div>
            )}
            <div 
              ref={(el) => { if (el) el.scrollTop = el.scrollHeight; }}
              style={{flex:1, overflowY:'auto', border:'1px solid #e2e8f0', borderRadius:'12px', padding:'12px', background:'#f8fafc', fontFamily:'monospace', fontSize:'12px'}}
            >
              {detailedLogs.length === 0 ? (
                <div style={{color:'#94a3b8', textAlign:'center', padding:'40px'}}>Henüz log yok</div>
              ) : (
                detailedLogs.slice(-200).map((log, idx) => (
                  <div key={idx} style={{
                    padding:'6px 8px',
                    borderBottom:'1px solid #e2e8f0',
                    display:'grid',
                    gridTemplateColumns:'140px 80px 120px 100px 1fr',
                    gap:'12px',
                    alignItems:'center'
                  }}>
                    <span style={{color:'#475569', fontSize:'11px'}}>{new Date(log.timestamp).toLocaleTimeString('tr-TR')}</span>
                    <span style={{fontWeight:'700', color:'#0f172a'}}>{log.device}</span>
                    <span style={{background:'#f0f9ff', color:'#0369a1', padding:'2px 6px', borderRadius:'4px', fontSize:'11px', textAlign:'center'}}>{log.protocol}</span>
                    <span style={{
                      padding:'4px 8px',
                      borderRadius:'4px',
                      fontSize:'11px',
                      fontWeight:'600',
                      textAlign:'center',
                      background: log.status === 'success' ? '#dcfce7' : log.status === 'error' ? '#fee2e2' : log.status === 'warning' ? '#fef3c7' : '#f0f9ff',
                      color: log.status === 'success' ? '#15803d' : log.status === 'error' ? '#991b1b' : log.status === 'warning' ? '#92400e' : '#0369a1'
                    }}>
                      {log.status}
                    </span>
            ...
            <div style={{marginTop:'12px', display:'flex', justifyContent:'flex-end', gap:'8px'}}>
              <button 
                onClick={() => {
                  const logText = detailedLogs.map(l => `[${new Date(l.timestamp).toLocaleTimeString()}] ${l.device} | ${l.protocol} | ${l.status} | ${l.message}`).join('\n');
                  navigator.clipboard.writeText(logText);
                }}
                style={{padding:'10px 16px', background:'#f0f9ff', color:'#0369a1', border:'1px solid #bae6fd', borderRadius:'8px', fontWeight:'700', cursor:'pointer'}}
              >
                📋 Kopyala
              </button>
              {loading ? (
                <button onClick={handleCancelDiscovery} style={{padding:'10px 16px', background:'#ef4444', color:'white', border:'none', borderRadius:'8px', fontWeight:'700', cursor:'pointer'}}>
                  İptal Et
                </button>
              ) : (
                <button onClick={() => setShowLogs(false)} style={{padding:'10px 16px', background:'var(--primary)', color:'white', border:'none', borderRadius:'8px', fontWeight:'700', cursor:'pointer'}}>
                  Kapat
                </button>
              )}
            </div>
...
const CertificateManagementView = ({ API_URL }) => {
  const [certificates, setCertificates] = useState([]);
  const [loading, setLoading] = useState(false);
  const [uploadForm, setUploadForm] = useState({ name: '', file_name: '', file_content: '' });
  ...
  const fetchCertificates = async () => {
    try {
      ...
      console.error('Sertifika listesi alınamadı:', e.message);
    }
  };
  ...
  const handleFileSelect = (event) => {
    ...
    reader.onload = (e) => {
      setUploadForm((prev) => ({
        ...prev,
        file_name: file.name,
        file_content: String(e.target?.result || ''),
  ...
  const handleUploadCertificate = async () => {
    if (!uploadForm.file_content) {
      alert('Lütfen sertifika dosyası seçin');
      return;
    }
    setLoading(true);
    try {
      await axios.post(`${API_URL}/certificates/upload`, uploadForm);
      setUploadForm({ name: '', file_name: '', file_content: '' });
      await fetchCertificates();
      alert('Sertifika başarıyla kaydedildi');
    } catch (e) {
      alert(e.response?.data?.error || `Yükleme hatası: ${e.message}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleFetchFromUrl = async () => {
    ...
    setLoading(true);
    try {
      await axios.post(`${API_URL}/certificates/fetch-url`, urlForm);
      setUrlForm({ name: '', url: '' });
      await fetchCertificates();
      alert('URL sertifikası başarıyla kaydedildi');
    } catch (e) {
      alert(e.response?.data?.error || `URL sertifika hatası: ${e.message}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDeleteCertificate = async (id) => {
    if (!window.confirm('Bu sertifika kaydını silmek istediğinize emin misiniz?')) return;
    try {
      await axios.delete(`${API_URL}/certificates/${id}`);
      await fetchCertificates();
    } catch (e) {
      alert(e.response?.data?.error || `Silme hatası: ${e.message}`);
    }
  };
...
const DiscoveryTemplatesManager = ({ API_URL }) => {
  const [templates, setTemplates] = useState([]);
  const [loading, setLoading] = useState(false);
  const [editingId, setEditingId] = useState(null);
  ...
  const fetchTemplates = async () => {
    setLoading(true);
    try {
      const res = await axios.get(`${API_URL}/discovery-templates`);
      setTemplates(res.data || []);
    } catch (e) {
      console.error('Templates fetch hatası:', e.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleEditClick = (template) => {
    ...
    setForm({
      ...
      snmp_enabled: template.snmp_enabled,
      ssh_enabled: template.ssh_enabled,
      lldp_command: template.lldp_command || '',
      cdp_command: template.cdp_command || '',
      arp_command: template.arp_command || '',
      ssh_pre_commands: Array.isArray(template.ssh_pre_commands) ? template.ssh_pre_commands.join('\n') : (template.ssh_pre_commands || ''),
  ...
  const handleReset = async () => {
    if (!window.confirm('Varsayılan şablonlar eklenecek. Emin misiniz?')) return;
    try {
      await axios.get(`${API_URL}/discovery-templates/reset`);
      fetchTemplates();
      alert('Varsayılan şablonlar başarıyla yüklendi.');
    } catch (e) { alert('Hata: ' + e.message); }
  };

  const handleSave = async () => {
    if (!form.vendor.trim()) {
      alert('Vendor adı gerekli');
      return;
    }

    try {
      if (editingId) {
        await axios.put(`${API_URL}/discovery-templates/${editingId}`, form);
        alert('Şablon güncellendi');
      } else {
        await axios.post(`${API_URL}/discovery-templates`, form);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`${API_URL}/discovery-templates/${id}`);
  ...
  if (loading) return <div style={{padding:'40px', textAlign:'center', color:'#64748b'}}><RefreshCw className="spin" /> Yükleniyor...</div>;

  return (
    <div className="discovery-kb-container fade-in" style={{ paddingBottom: '40px' }}>
      <div style={{ background: 'white', borderRadius: '24px', padding: '30px', border: '1px solid #e2e8f0', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)', marginBottom: '30px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '20px' }}>
          <div>
            <h3 style={{ margin: 0, fontSize: '1.5rem', fontWeight: '800', color: '#1e293b' }}>Keşif Şablonları (Discovery KB)</h3>
            <p style={{ margin: '5px 0 0', fontSize: '14px', color: '#64748b' }}>Cihaz türlerine göre akıllı keşif (LLDP/CDP/ARP) komutlarını ve önceliklerini yönetin.</p>
          </div>
          <div style={{ display: 'flex', gap: '12px' }}>
            <button onClick={handleReset} style={{ background: '#f8fafc', color: '#475569', border: '1px solid #e2e8f0', padding: '12px 20px', borderRadius: '14px', fontWeight: '700', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', transition: 'all 0.2s' }} onMouseEnter={e => e.currentTarget.style.background = '#f1f5f9'} onMouseLeave={e => e.currentTarget.style.background = '#f8fafc'}>
              <RefreshCw size={18} /> Varsayılanlar
            </button>
            <button
              onClick={() => { setShowForm(!showForm); setEditingId(null); setForm({ vendor: '', model: '', priority: 100, snmp_enabled: true, ssh_enabled: true, lldp_command: '', cdp_command: '', arp_command: '', ssh_pre_commands: '', description: '', enabled: true }); }}
              style={{ background: showForm ? '#ef4444' : 'var(--primary)', color: 'white', border: 'none', padding: '12px 24px', borderRadius: '14px', fontWeight: '800', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }}
            >
              {showForm ? <X size={20} /> : <Plus size={20} />}
              {showForm ? 'Kapat' : 'Yeni Şablon Ekle'}
            </button>
          </div>
        </div>

        {showForm && (
          <div style={{ background: '#f8fafc', padding: '25px', borderRadius: '20px', border: '1px solid #e2e8f0', marginTop: '25px', animation: 'slideDown 0.3s ease-out' }}>
            <h4 style={{ margin: '0 0 20px', fontSize: '1.1rem', fontWeight: '800', color: '#334155' }}>{editingId ? 'Şablonu Düzenle' : 'Yeni Keşif Şablonu'}</h4>
            
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '20px', marginBottom: '20px' }}>
              <div>
                <label style={{ display: 'block', fontSize: '12px', fontWeight: '800', color: '#64748b', marginBottom: '8px', textTransform: 'uppercase' }}>Vendor (Üretici) *</label>
                <input
                  type="text"
                  value={form.vendor}
                  onChange={(e) => setForm({ ...form, vendor: e.target.value })}
                  placeholder="örn: Cisco, Huawei, Palo Alto..."
                  style={{ width: '100%', padding: '12px', border: '1px solid #cbd5e1', borderRadius: '12px', fontSize: '14px', fontWeight: '600' }}
                  required
                />
              </div>
              <div>
                <label style={{ display: 'block', fontSize: '12px', fontWeight: '800', color: '#64748b', marginBottom: '8px', textTransform: 'uppercase' }}>Model (Opsiyonel)</label>
                <input
                  type="text"
                  value={form.model}
                  onChange={(e) => setForm({ ...form, model: e.target.value })}
                  placeholder="örn: All, Catalyst 9000..."
                  style={{ width: '100%', padding: '12px', border: '1px solid #cbd5e1', borderRadius: '12px', fontSize: '14px', fontWeight: '600' }}
                />
              </div>
              <div>
                <label style={{ display: 'block', fontSize: '12px', fontWeight: '800', color: '#64748b', marginBottom: '8px', textTransform: 'uppercase' }}>Tarama Önceliği</label>
                <input
                  type="number"
                  value={form.priority}
            ...
            <div style={{ background: 'white', padding: '20px', borderRadius: '16px', border: '1px solid #e2e8f0', marginBottom: '20px' }}>
              <label style={{ display: 'block', fontSize: '12px', fontWeight: '800', color: '#64748b', marginBottom: '15px', textTransform: 'uppercase' }}>SSH Keşif Komutları</label>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '15px' }}>
                <div>
                  <label style={{ display: 'block', fontSize: '11px', fontWeight: '700', color: '#94a3b8', marginBottom: '5px' }}>LLDP Komutu</label>
                  <input
                    type="text"
                    value={form.lldp_command}
                    onChange={(e) => setForm({ ...form, lldp_command: e.target.value })}
                    placeholder="örn: show lldp neighbors detail"
                    style={{ width: '100%', padding: '10px', border: '1px solid #e2e8f0', borderRadius: '8px', fontSize: '13px' }}
                  />
                </div>
                <div>
                  <label style={{ display: 'block', fontSize: '11px', fontWeight: '700', color: '#94a3b8', marginBottom: '5px' }}>CDP Komutu</label>
                  <input
                    type="text"
                    value={form.cdp_command}
                    onChange={(e) => setForm({ ...form, cdp_command: e.target.value })}
                    placeholder="örn: show cdp neighbors detail"
                    style={{ width: '100%', padding: '10px', border: '1px solid #e2e8f0', borderRadius: '8px', fontSize: '13px' }}
                  />
                </div>
                <div>
                  <label style={{ display: 'block', fontSize: '11px', fontWeight: '700', color: '#94a3b8', marginBottom: '5px' }}>ARP Komutu</label>
                  <input
                    type="text"
                    value={form.arp_command}
                    onChange={(e) => setForm({ ...form, arp_command: e.target.value })}
                    placeholder="örn: show ip arp"
                    style={{ width: '100%', padding: '10px', border: '1px solid #e2e8f0', borderRadius: '8px', fontSize: '13px' }}
                  />
                </div>
              </div>
              <div style={{ marginTop: '15px' }}>
                <label style={{ display: 'block', fontSize: '11px', fontWeight: '700', color: '#94a3b8', marginBottom: '5px' }}>Ön Hazırlık Komutları (Alt satıra geçerek yazın)</label>
                <textarea
                  value={form.ssh_pre_commands}
                  onChange={(e) => setForm({ ...form, ssh_pre_commands: e.target.value })}
                  placeholder={'terminal length 0\nterminal width 200'}
                  rows="3"
                  style={{ width: '100%', padding: '10px', border: '1px solid #e2e8f0', borderRadius: '8px', fontSize: '13px', fontFamily: 'monospace' }}
                />
              </div>
            </div>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '25px', marginBottom: '25px' }}>
              <label style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer', fontSize: '14px', fontWeight: '700', color: '#334155' }}>
                <input type="checkbox" checked={form.snmp_enabled} onChange={(e) => setForm({ ...form, snmp_enabled: e.target.checked })} style={{ width: '18px', height: '18px' }} />
                SNMP Taraması Aktif
              </label>
              <label style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer', fontSize: '14px', fontWeight: '700', color: '#334155' }}>
                <input type="checkbox" checked={form.ssh_enabled} onChange={(e) => setForm({ ...form, ssh_enabled: e.target.checked })} style={{ width: '18px', height: '18px' }} />
                SSH Taraması Aktif
              </label>
              <label style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer', fontSize: '14px', fontWeight: '700', color: '#334155' }}>
                <input type="checkbox" checked={form.enabled} onChange={(e) => setForm({ ...form, enabled: e.target.checked })} style={{ width: '18px', height: '18px' }} />
                Şablonu Kullanıma Al
              </label>
            </div>

            <div style={{ display: 'flex', gap: '12px' }}>
              <button onClick={handleSave} style={{ background: 'var(--primary)', color: 'white', border: 'none', padding: '14px 30px', borderRadius: '14px', fontWeight: '800', cursor: 'pointer', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }}>
                {editingId ? 'Değişiklikleri Kaydet' : 'Şablonu Oluştur'}
              </button>
              <button onClick={() => setShowForm(false)} style={{ background: 'white', color: '#64748b', border: '1px solid #cbd5e1', padding: '14px 30px', borderRadius: '14px', fontWeight: '700', cursor: 'pointer' }}>
                İptal
              </button>
            </div>
          </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(350px, 1fr))', gap: '20px' }}>
        {templates.map((t) => (
          <div key={t.id} style={{ background: 'white', borderRadius: '20px', border: '1px solid #e2e8f0', overflow: 'hidden', boxShadow: '0 2px 4px rgba(0,0,0,0.02)', transition: 'transform 0.2s', opacity: t.enabled ? 1 : 0.6 }} onMouseEnter={e => e.currentTarget.style.transform = 'translateY(-2px)'} onMouseLeave={e => e.currentTarget.style.transform = 'none'}>
            <div style={{ padding: '20px', background: '#f8fafc', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                  <Shield size={16} color="var(--primary)" />
                  <span style={{ fontSize: '1.1rem', fontWeight: '800', color: '#0f172a' }}>{t.vendor}</span>
                </div>
                <div style={{ fontSize: '13px', color: '#64748b', fontWeight: '600' }}>{t.model || 'Tüm Modeller'}</div>
              </div>
              <div style={{ background: 'white', padding: '4px 10px', borderRadius: '8px', border: '1px solid #e2e8f0', fontSize: '11px', fontWeight: '800', color: '#64748b' }}>
                PRIORITY: {t.priority}
              </div>
            </div>
            ...
            <div style={{ padding: '20px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '20px' }}>
                <div style={{ background: t.snmp_enabled ? '#f0fdf4' : '#fef2f2', padding: '10px', borderRadius: '12px', border: `1px solid ${t.snmp_enabled ? '#dcfce7' : '#fee2e2'}`, textAlign: 'center' }}>
                  <div style={{ fontSize: '10px', fontWeight: '800', color: t.snmp_enabled ? '#166534' : '#991b1b', textTransform: 'uppercase', marginBottom: '2px' }}>SNMP</div>
                  <div style={{ fontSize: '13px', fontWeight: '800', color: t.snmp_enabled ? '#15803d' : '#b91c1c' }}>{t.snmp_enabled ? 'AKTİF' : 'PASİF'}</div>
                </div>
                <div style={{ background: t.ssh_enabled ? '#f0fdf4' : '#fef2f2', padding: '10px', borderRadius: '12px', border: `1px solid ${t.ssh_enabled ? '#dcfce7' : '#fee2e2'}`, textAlign: 'center' }}>
                  <div style={{ fontSize: '10px', fontWeight: '800', color: t.ssh_enabled ? '#166534' : '#991b1b', textTransform: 'uppercase', marginBottom: '2px' }}>SSH</div>
                  <div style={{ fontSize: '13px', fontWeight: '800', color: t.ssh_enabled ? '#15803d' : '#b91c1c' }}>{t.ssh_enabled ? 'AKTİF' : 'PASİF'}</div>
                </div>
              </div>

              <div style={{ background: '#fafafa', padding: '15px', borderRadius: '12px', border: '1px dashed #e2e8f0', marginBottom: '20px' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <div style={{ display: 'flex', gap: '10px', fontSize: '12px' }}>
                    <span style={{ color: '#94a3b8', fontWeight: '700', width: '40px' }}>LLDP:</span>
                    <span style={{ color: '#334155', fontFamily: 'monospace' }}>{t.lldp_command || '-'}</span>
                  </div>
                  <div style={{ display: 'flex', gap: '10px', fontSize: '12px' }}>
                    <span style={{ color: '#94a3b8', fontWeight: '700', width: '40px' }}>CDP:</span>
                    <span style={{ color: '#334155', fontFamily: 'monospace' }}>{t.cdp_command || '-'}</span>
                  </div>
                  <div style={{ display: 'flex', gap: '10px', fontSize: '12px' }}>
                    <span style={{ color: '#94a3b8', fontWeight: '700', width: '40px' }}>ARP:</span>
                    <span style={{ color: '#334155', fontFamily: 'monospace' }}>{t.arp_command || '-'}</span>
                  </div>
                </div>
              </div>

              <div style={{ display: 'flex', gap: '10px' }}>
                <button onClick={() => handleEditClick(t)} style={{ flex: 1, background: '#f0f9ff', color: '#0369a1', border: 'none', padding: '10px', borderRadius: '10px', fontWeight: '700', cursor: 'pointer', fontSize: '13px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }} onMouseEnter={e => e.currentTarget.style.background = '#e0f2fe'} onMouseLeave={e => e.currentTarget.style.background = '#f0f9ff'}>
                  <Edit size={14} /> Düzenle
                </button>
                <button onClick={() => handleDelete(t.id)} style={{ flex: 1, background: '#fef2f2', color: '#991b1b', border: 'none', padding: '10px', borderRadius: '10px', fontWeight: '700', cursor: 'pointer', fontSize: '13px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }} onMouseEnter={e => e.currentTarget.style.background = '#fee2e2'} onMouseLeave={e => e.currentTarget.style.background = '#fef2f2'}>
                  <Trash2 size={14} /> Sil
                </button>
              </div>
            </div>
        ...
        {templates.length === 0 && (
          <div style={{ gridColumn: '1/-1', textAlign: 'center', padding: '60px', color: '#94a3b8' }}>
            <Database size={48} style={{ marginBottom: '15px', opacity: 0.5 }} />
            <div>Henüz keşif şablonu tanımlanmamış.</div>
          </div>
        )}
      </div>
    </div>
...
const NetworkScanView = ({ API_URL }) => {
  ...
  const generateIPs = () => {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      let currentValues = [];
      if (octet.type === 'standard') {
        currentValues.push(octet.value);
      } else {
        const s = parseInt(octet.start);
        const e = parseInt(octet.end);
        for (let v = s; v <= e; v++) {
...
const TopologyView = ({ API_URL }) => {
  ...
  const handleBgContextMenu = (e) => {
    ...
    setContextMenu({ x: e.clientX, y: e.clientY, type: 'bg', canvasX: x, canvasY: y });
  };
  ...
  return (
    <div className="topology-editor fade-in" style={containerStyle}>
      ...
      <div style={{flex:1, background:'#f1f5f9', borderRadius:isFullScreen?0:'24px', border:isFullScreen?'none':'2px dashed #e2e8f0', position:'relative', overflow:'hidden'}} onMouseMove={onMouseMove} onMouseUp={handleMouseUp} onMouseDown={handleBgMouseDown} onWheel={handleWheel}>
        <svg ref={svgRef} width="100%" height="100%" onClick={handleSvgClick} onContextMenu={handleBgContextMenu} style={{cursor: isPanning ? 'grabbing' : (tool==='move'?'default': tool==='delete'?'no-drop' : 'crosshair'), background: 'radial-gradient(#cbd5e1 1px, transparent 1px)', backgroundSize: `${20 * transform.k}px ${20 * transform.k}px`, backgroundPosition: `${transform.x}px ${transform.y}px`}}>
      ...
      {contextMenu && (
        <div style={{position:'fixed', top:contextMenu.y, left:contextMenu.x, background:'white', border:'1px solid #e2e8f0', borderRadius:'12px', padding:'8px', boxShadow:'0 10px 15px -3px rgba(0,0,0,0.1)', zIndex:4000, minWidth:'180px'}}>
          {contextMenu.type === 'node' ? (
            ...
            <>
              <button 
                onClick={() => pasteNode(contextMenu.canvasX, contextMenu.canvasY)} 
                disabled={!clipboard}
                style={{
                  display:'flex', alignItems:'center', gap:'10px', width:'100%', padding:'10px 15px', border:'none', background:'none', 
                  cursor: clipboard ? 'pointer' : 'default', borderRadius:'8px', fontSize:'13px', fontWeight:'600', 
                  color: clipboard ? '#1e293b' : '#94a3b8', opacity: clipboard ? 1 : 0.5
                }} 
                onMouseOver={e=> clipboard && (e.currentTarget.style.background='#f1f5f9')} 
      ...
      <aside className="app-sidebar">
        ...
        <nav className="sidebar-nav">
          ...
          <div className="nav-group">
            <div className={`nav-item ${currentView === 'firewallAnalysis' || currentView === 'hitcounts' ? 'active' : ''}`} onClick={() => setCurrentView('firewallAnalysis')} style={{cursor:'pointer'}}>
              <Shield size={20} /><span>Firewall Analizi</span>
            </div>
            {(currentView === 'firewallAnalysis' || currentView === 'hitcounts') && (
              <div className="sub-menu">
                <div className={`sub-nav-item ${currentView === 'firewallAnalysis' ? 'active' : ''}`} onClick={(e) => { e.stopPropagation(); setCurrentView('firewallAnalysis'); }}>
          ...
          <div className={`nav-item ${currentView === 'howto' ? 'active' : ''}`} onClick={() => setCurrentView('howto')} style={{cursor:'pointer'}}><FileText size={20} /><span>Nasıl Yapılır</span></div>
      ...
      <div className="app-main">
        ...
        <div className="content-area">
          {currentView === 'recent' ? (
            ...
            <CertificateManagementView API_URL={API_URL} />
                  ...
                  <>
                    ...
                    <div style={{display:'flex', gap:'10px', alignItems:'center', flexWrap:'wrap'}}>
                      <select value={selectedScanDeviceId} onChange={(e) => setSelectedScanDeviceId(e.target.value)} style={{padding:'12px', borderRadius:'10px', border:'1px solid #e2e8f0', minWidth:'320px', background:'white', fontSize:'14px'}}>
                        {devices.filter(d => d.api_template_id).length === 0 && <option value="">API destekli cihaz bulunamadı</option>}
                        {devices.filter(d => d.api_template_id).map((d) => (
                          <option key={d.id} value={d.id}>{d.name || d.ip_address} ({d.ip_address})</option>
                        ))}
                      </select>
                      <button onClick={handleDeviceApiScan} disabled={isDeviceScanLoading || devices.filter(d => d.api_template_id).length === 0} style={{background:'var(--primary)', color:'white', border:'none', padding:'13px 25px', borderRadius:'12px', fontWeight:'700', cursor: isDeviceScanLoading ? 'not-allowed' : 'pointer', opacity: isDeviceScanLoading ? 0.7 : 1}}>
                        {isDeviceScanLoading ? 'Konfigürasyon Çekiliyor...' : 'Analizi Başlat'}
                      </button>
                      <button onClick={() => setScanSource(null)} style={{padding:'12px 18px', borderRadius:'12px', border:'1px solid #e2e8f0', background:'white', cursor:'pointer', fontWeight:'700'}}>Yöntemi Değiştir</button>
                    </div>
                    {devices.filter(d => d.api_template_id).length === 0 && <p style={{margin:'10px 0 0', color:'#ef4444', fontSize:'13px', fontWeight:'600'}}>Bu özellik için önce "Cihaz Yönetimi" menüsünden <strong>API Template</strong> tanımlı bir cihaz eklemelisiniz.</p>}
                  </>
                )}
              </div>
